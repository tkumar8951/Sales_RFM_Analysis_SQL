...
-- 2005 has least sales among all the years

select distinct MONTH_ID from sales_data_sample
where YEAR_ID=2005
order by MONTH_ID asc

--They operated for onbly 5 months in 2005
...
into #rrfm
from #rfm
select * from #rrfm where rfm_segment = 'Loyal' order by rfm_frequency desc, rfm_MonetaryValue desc
...
select distinct ORDERNUMBER, STUFF(
				...
				where ORDERNUMBER in 
					(
						select ORDERNUMBER
						from (
								select ORDERNUMBER, COUNT(*) as no_of_orders
								from sales_data_sample
